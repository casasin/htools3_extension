...
def autoUnicode(glyph, customUnicodes={}, verbose=False, indent=0):
    '''
    ...
    if glyph.name is not None:
        ...
        if verbose:
            if uniInt:
                print('%ssetting unicode for %s: %s (%s) [%s]...' % ('\t' * indent, glyph.name, uniHex, uniInt, uniSrc))
            else:
                print('%sno unicode for %s.' % ('\t' * indent, glyph.name))
